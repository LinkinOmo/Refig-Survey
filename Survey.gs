            ...
            return { success: true, data: {
                ...
                fullData: {
                    general: {
                        ...
                        brokenUnits: brokenUnits,
                        brokenDetail: gv('Broken Units Detail') || '',
    ...
    try {
        ...
        unitNumList.forEach(function(n) {
            ...
            brokenUnitDetail["unit" + ni] = {
                equipId:     form["RefUnit" + ni + "_EquipId"]   || "",
        ...
        try {
            ...
            // photoPrefixKey: attachment key prefix e.g. 'ref_open' → per-unit keys are 'ref_open_1', 'ref_open_2'...
            function unitRowsHtml(prefixKey, qty, startIdx, photoPrefixKey) {
                ...
                for (var u = start; u < start + qty; u++) {
                    ...
                    s += '</table>';
                    // Per-unit photos (key format: photoPrefixKey_globalUnitIndex)
                    if (photoPrefixKey) s += sectionPhotos(photoPrefixKey + '_' + u);
                    s += '</div>';
                }
                return s;
            }
            ...
            if (_openQty > 0)
                htmlBody += sectionCard('2.1 ตู้แช่ OPEN (' + _openQty + ' ตู้)', '#0369a1',
                    unitRowsHtml('Ref Open ', _openQty, 1, 'ref_open'), null);

            // 2.2 ตู้ทำความเย็นเครื่องดื่ม — per-unit photos embedded inside each sub-type
            var _bevBodyHtml = '';
            if (_pluginQty > 0) _bevBodyHtml += '<p style="margin:0 0 6px 0;font-size:12px;font-weight:700;color:#0f766e;">Plugin (' + _pluginQty + ' ตู้)</p>' + unitRowsHtml('Ref Bev Plugin ', _pluginQty, 1, 'ref_bev_plugin');
            if (_walkinQty > 0) _bevBodyHtml += '<p style="margin:' + (_pluginQty > 0 ? '10px' : '0') + ' 0 6px 0;font-size:12px;font-weight:700;color:#0f766e;">Walk-in (' + _walkinQty + ' ตู้)</p>' + unitRowsHtml('Ref Bev Walk In ', _walkinQty, 1, 'ref_bev_walkin');
            if (_remoteQty > 0) _bevBodyHtml += '<p style="margin:' + (_pluginQty + _walkinQty > 0 ? '10px' : '0') + ' 0 6px 0;font-size:12px;font-weight:700;color:#0f766e;">Remote (' + _remoteQty + ' ตู้)</p>' + unitRowsHtml('Ref Bev Remote ', _remoteQty, 1, 'ref_bev_remote');
            if (_bevBodyHtml)
            ...
            if (_icecreamQty > 0)
                htmlBody += sectionCard('2.4 ตู้แช่ไอศกรีม (' + _icecreamQty + ' ตู้)', '#7c3aed',
                    unitRowsHtml('Ref Ice Cream ', _icecreamQty, 1, 'ref_icecream'), null);

            // 2.5 น้ำแข็ง
            if (_iceQty > 0)
                htmlBody += sectionCard('2.5 ตู้แช่น้ำแข็ง (' + _iceQty + ' ตู้)', '#0891b2',
                    unitRowsHtml('Ref Ice ', _iceQty, 1, 'ref_ice'), null);

            // 2.6 Stainless 4 Door
            if (_ss4DoorQty > 0)
                htmlBody += sectionCard('2.6 ตู้ 4 ประตู Stainless (' + _ss4DoorQty + ' ตู้)', '#475569',
                    unitRowsHtml('Ref Ss4Door ', _ss4DoorQty, 1, 'ref_ss4door'), null);
...
// ── Ref Broken History Logger ─────────────────────────────────────────────────
// Writes one row per broken unit into Ref_Broken_History when a case is closed.
// Called only when the record transitions from a non-Closed status → Closed.
function _logRefBrokenHistory(rowData, headers, brokenDetail, closedBy, closedAt) {
    try {
        var ss = SpreadsheetApp.getActiveSpreadsheet();
        var logSheet = ss.getSheetByName('Ref_Broken_History');

        var LOG_HEADERS = [
            'Log ID', 'Logged At', 'Survey ID',
            'Branch Code', 'Branch Name', 'Survey Date', 'Reporter Name',
            'Unit Number', 'Equipment ID', 'Unit Type', 'Asset No',
            'อุณหภูมิ (Temp)', 'น้ำแข็งสะสม (Frost)', 'คอมเพรสเซอร์ (Compressor)',
            'พัดลมคอยล์เย็น (EvapFan)', 'พัดลมคอนเดนเซอร์ (CondFan)',
            'ขอบยางประตู (Gasket)', 'ท่อระบาย (Drain)',
            'น้ำยาทำความเย็น (Refrigerant)', 'คอยล์ร้อน (Condenser)',
            'คอนโทรลเลอร์ (Controller)', 'Note', 'Closed By', 'Closed Date'
        ];

        if (!logSheet) {
            logSheet = ss.insertSheet('Ref_Broken_History');
            logSheet.appendRow(LOG_HEADERS);
            logSheet.setFrozenRows(1);
            var hdrRng = logSheet.getRange(1, 1, 1, LOG_HEADERS.length);
            hdrRng.setBackground('#1e40af');
        ...
        // Map header names → array index in the source row
        var cm = {};
        headers.forEach(function(h, i) { cm[String(h).trim()] = i; });
        ...
        var surveyId   = getField('ID');
        var branchCode = getField('Branch Code');
        var branchName = getField('Branch Name');
        var surveyDate = getField('Timestamp');
        var reporter   = getField('Reporter Name') || getField('Reporter Email');

        var ts = closedAt.getTime();
        var logRows = [];

        var unitKeys = Object.keys(brokenDetail);
        unitKeys.sort(); // keep order deterministic
        unitKeys.forEach(function(unitKey) {
            if (!brokenDetail.hasOwnProperty(unitKey)) return;
            var u = brokenDetail[unitKey];
            // Handle both old format (key="unit1") and new format (key=equipId like "BKK001-OC01")
            var unitNum = u.unitNum !== undefined ? String(u.unitNum) : unitKey.replace(/^unit/, '');
            var unitEquipId = u.equipId || unitKey;
            var logId = 'BHL-' + ts + '-U' + unitNum;

            logRows.push([
                logId, closedAt, surveyId,
                branchCode, branchName, surveyDate, reporter,
                unitNum, unitEquipId, u.type || '', u.assetNo || '',
                u.temp || '', u.frost || '', u.compressor || '',
                u.evapFan || '', u.condFan || '',
                u.gasket || '', u.drain || '',
                u.refrigerant || '', u.condenser || '',
                u.controller || '', u.note || '',
                closedBy, closedAt
            ]);
        });
        ...
        if (logRows.length > 0) {
            logSheet.getRange(logSheet.getLastRow() + 1, 1, logRows.length, LOG_HEADERS.length)
                    .setValues(logRows);
        }
    } catch (e) {
        Logger.log('_logRefBrokenHistory error: ' + e.toString());
    }
}

// ── Query broken-unit history by Equipment ID ────────────────────────────────
function getRefBrokenHistory(equipId) {
    try {
        var ss = SpreadsheetApp.getActiveSpreadsheet();
        var sheet = ss.getSheetByName('Ref_Broken_History');
        if (!sheet || sheet.getLastRow() < 2) return { success: true, rows: [] };

        var data = sheet.getDataRange().getValues();
        var headers = data[0].map(function(h){ return String(h).trim(); });
        var equipIdCol = headers.indexOf('Equipment ID');
        if (equipIdCol === -1) return { success: true, rows: [] };

        var rows = [];
        var target = String(equipId || '').trim();
        for (var i = 1; i < data.length; i++) {
            if (String(data[i][equipIdCol]).trim() === target) {
                var row = {};
                headers.forEach(function(h, idx) {
                    var v = data[i][idx];
        ...
        rows.sort(function(a, b) {
            var ta = a['Logged At'] ? new Date(a['Logged At']).getTime() : 0;
            var tb = b['Logged At'] ? new Date(b['Logged At']).getTime() : 0;
            return tb - ta;
        });
...
function updateRefUnitFrameRust(id, fieldKey, value, clientEmail) {
    try {
        ...
        if (!adminCheck.isAdmin && !adminCheck.isScheduleAdmin && !adminCheck.isSMF) {
            return { success: false, error: "ไม่มีสิทธิ์แก้ไขข้อมูลนี้" };
        }
        // Validate fieldKey to prevent arbitrary column injection
        if (!fieldKey || !/^Ref .+ Frame Rust_\d+$/.test(String(fieldKey))) {
            return { success: false, error: "Invalid field key" };
        }
        // Validate value
        var allowed = ['', 'ไม่มีสนิม', 'มีสนิม'];
        ...
        if (!colMap[fieldKey]) {
            ...
            colMap[fieldKey] = headers.length;
            sheet.getRange(1, headers.length).setValue(fieldKey);
        }
        sheet.getRange(rowIndex, colMap[fieldKey]).setValue(value);
        return { success: true };
    } catch (e) {
        return { success: false, error: e.toString() };
    }
...
function updateRefSurveyRecord(form) {
    try {
        ...
        // Capture previous status to detect Close transition
        ...
        if (form.status === 'Closed' && prevStatus !== 'Closed' && form.hasBrokenRef === 'Yes') {
            try {
                var _bd = (typeof form.brokenDetail === 'string')
                    ...
                    : (form.brokenDetail || {});
                if (Object.keys(_bd).length > 0) {
                    _logRefBrokenHistory(data[rowIndex - 1], headers, _bd, userEmail, closedAt || new Date());
                }
            } catch (_bdE) {
                Logger.log('updateRefSurveyRecord — broken history log error: ' + _bdE);
            }
        }
...
// ── Log Aircon broken-unit symptom history on Close ──────────────────────────
// rowData  : raw data array for the survey row
// headers  : normalized header strings
// brokenUnitsStr : comma-separated broken unit numbers e.g. "1, 3"
// closedBy : email string
// closedAt : Date object
function _logAcBrokenHistory(rowData, headers, brokenUnitsStr, closedBy, closedAt) {
    try {
        var ss = SpreadsheetApp.getActiveSpreadsheet();
        var logSheet = ss.getSheetByName('Ac_Broken_History');

        var LOG_HEADERS = [
            'Log ID', 'Logged At', 'Survey ID',
            'Branch Code', 'Branch Name', 'Survey Date', 'Reporter Name',
            'Unit Number', 'Equipment ID', 'Unit Type', 'Brand', 'Asset No',
            'ความสะอาดแผงคอยล์ (Filter)', 'ความเย็น/ลม (Cooling)',
            'การระบายน้ำ (Drainage)', 'เสียงการทำงาน (Noise)',
            'สภาพอุปกรณ์ (Condition)', 'Note', 'Closed By', 'Closed Date'
        ];

        if (!logSheet) {
            logSheet = ss.insertSheet('Ac_Broken_History');
            logSheet.appendRow(LOG_HEADERS);
            logSheet.setFrozenRows(1);
            var hdrRng = logSheet.getRange(1, 1, 1, LOG_HEADERS.length);
            hdrRng.setBackground('#1e40af');
        ...
        var ts = closedAt.getTime();
        var unitNums = String(brokenUnitsStr || '').split(',').map(function(s) { return s.trim(); }).filter(Boolean);
        var logRows = [];

        unitNums.forEach(function(n) {
            var unitType  = getField('Unit ' + n + ' Type');
            var unitBrand = getField('Unit ' + n + ' Brand');
            var assetNo   = getField('Unit ' + n + ' Asset No');
            var unitNote  = getField('Unit ' + n + ' Note');
            var filter    = getField('Filter Cleanliness ' + n);
            var cooling   = getField('Cooling Performance ' + n);
            var drainage  = getField('Drainage Leakage ' + n);
            var noise     = getField('Operational Noise ' + n);
            var condition = getField('Equipment Condition ' + n);

            var typeCode = acTypeCodes[unitType] || 'AC';
            var padded   = ('0' + n).slice(-2);
            var equipId  = branchCode ? (branchCode + '-' + typeCode + padded) : (typeCode + padded);
            var logId    = 'ACH-' + ts + '-U' + n;

            logRows.push([
                logId, closedAt, surveyId,
                branchCode, branchName, surveyDate, reporter,
                n, equipId, unitType || '', unitBrand || '', assetNo || '',
                filter || '', cooling || '',
                drainage || '', noise || '',
                condition || '', unitNote || '',
                closedBy, closedAt
        ...
        Logger.log('_logAcBrokenHistory error: ' + e.toString());
    }
}

// ── Query Aircon broken-unit history by Equipment ID ─────────────────────────
function getAcBrokenHistory(equipId) {
    try {
        var ss = SpreadsheetApp.getActiveSpreadsheet();
        var sheet = ss.getSheetByName('Ac_Broken_History');
        if (!sheet || sheet.getLastRow() < 2) return { success: true, rows: [] };
...
function updateAcSurveyRecord(form) {
    try {
        ...
        setVal("Comments",    form.comments);
        setVal("Broken Units", form.brokenUnits);
        if (form.brokenDetail !== undefined && form.brokenDetail !== null) {
            setVal("Broken Units Detail", form.brokenDetail);
        }
        ...
        if (form.status === 'Closed' && prevStatus !== 'Closed') {
            ...
            try {
                if (form.brokenDetail) {
                    var _ac_bd = (typeof form.brokenDetail === 'string') ? JSON.parse(form.brokenDetail || '{}') : form.brokenDetail;
                    _ac_brokenNumsStr = Object.values(_ac_bd).map(function(u) { return u.unitNum; }).filter(Boolean).join(', ');
                }
                if (!_ac_brokenNumsStr) _ac_brokenNumsStr = String(form.brokenUnits || '').trim();
            } catch(_e) { _ac_brokenNumsStr = String(form.brokenUnits || '').trim(); }
            if (_ac_brokenNumsStr) {
                try {
                    _logAcBrokenHistory(data[rowIndex - 1], headers, _ac_brokenNumsStr, userEmail, closedAt || new Date());
                } catch (_acHE) {
                    Logger.log('updateAcSurveyRecord — broken history log error: ' + _acHE);
                }
            }
        }
...
//  Products: SC=Soft Serve, JT=Jet Spray, SH=Slushy, MK=Ice Maker, WF=Water Filter
// =============================================================================

function _invalidateNpSlimCache() {
    try {
        var cache = CacheService.getScriptCache();
        var keys = ['NP_SLIM_V1_n', 'NP_SLIM_V2_n', 'COMBINED_TREND_V1'];
        var v1 = cache.get('NP_SLIM_V1_n'); if (v1) { for (var i = 0; i < parseInt(v1,10); i++) keys.push('NP_SLIM_V1_' + i); }
        var v2 = cache.get('NP_SLIM_V2_n'); if (v2) { for (var j = 0; j < parseInt(v2,10); j++) keys.push('NP_SLIM_V2_' + j); }
        cache.removeAll(keys);
    } catch(e) {}
}

// ── Slim list (cached) ────────────────────────────────────────────────────────
function getNpSurveyListSlim() {
    try {
        try {
            var _sc = CacheService.getScriptCache();
            var _nStr = _sc.get('NP_SLIM_V2_n');
            if (_nStr !== null) {
                var _n = parseInt(_nStr, 10), _json = '', _ok = true;
                for (var _ci = 0; _ci < _n; _ci++) {
                    var _ch = _sc.get('NP_SLIM_V2_' + _ci);
                    if (_ch === null) { _ok = false; break; }
                    _json += _ch;
                }
                if (_ok) return { success: true, data: JSON.parse(_json), isAdmin: false, isSMF: false };
            }
        } catch(_scE) {}

        var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('NewProduct_Survey_Database');
        if (!sheet || sheet.getLastRow() < 2) return { success: true, data: [], isAdmin: false, isSMF: false };

        var lastCol  = sheet.getLastColumn();
        var lastRow  = sheet.getLastRow();
        var headers  = sheet.getRange(1, 1, 1, lastCol).getValues()[0];

        var LIST_KEYS = ['ID','Timestamp','Branch Code','Branch Name','Reporter Name','Reporter Email','Status','Has Broken NP','Broken Units','DM Area','CM Area','AMM MTN','SC Qty','JT Qty','SH Qty','MK Qty','WF Qty'];
        var keyIdx = {};
        LIST_KEYS.forEach(function(k) { keyIdx[k] = headers.indexOf(k); });

        var maxNeededIdx = -1;
        LIST_KEYS.forEach(function(k) { if (keyIdx[k] > maxNeededIdx) maxNeededIdx = keyIdx[k]; });
        if (maxNeededIdx < 0) return { success: true, data: [], isAdmin: false, isSMF: false };
        ...
        try {
            ...
            var _store = {}, _nc = 0;
            for (var _cs = 0; _cs < _cj.length; _cs += 90000) {
                _store['NP_SLIM_V2_' + _nc++] = _cj.substring(_cs, _cs + 90000);
            }
            _store['NP_SLIM_V2_n'] = String(_nc);
            _co.putAll(_store, 300);
        } catch(_we) {}

        return { success: true, data: results, isAdmin: false, isSMF: false };
    } catch(e) {
        return { success: false, error: e.toString() };
    }
}
...
// ── Single full record ────────────────────────────────────────────────────────
function getNpSurveyRecordById(id) {
    try {
        var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('NewProduct_Survey_Database');
        if (!sheet) return { success: false, error: 'Sheet not found' };
        var lastRow = sheet.getLastRow();
        if (lastRow < 2) return { success: false, error: 'Record not found' };
...
function updateNpSurveyRecord(form) {
    try {
        ...
        setVal('Status',            form.status);
        setVal('Comments',          form.comments);
        setVal('Has Broken NP',     form.hasBrokenNP);
        setVal('Broken Units',      form.brokenUnits);
        setVal('Broken Units Detail', form.brokenDetail);
        ...
        if (form.unitQty) {
            var uq = (typeof form.unitQty === 'string') ? JSON.parse(form.unitQty) : form.unitQty;
            for (var qk in uq) { if (uq.hasOwnProperty(qk)) setVal(qk, uq[qk]); }
        }
        if (form.unitFields) {
            var uf = (typeof form.unitFields === 'string') ? JSON.parse(form.unitFields) : form.unitFields;
            for (var fk in uf) { if (uf.hasOwnProperty(fk)) setVal(fk, uf[fk]); }
        }
...
function submitNpSurvey(form) {
    try {
        ...
        if (!sheet) {
            sheet = ss.insertSheet('NewProduct_Survey_Database');
        }

        var NP_PRODUCTS = [
            { code: 'SC', prefix: 'NP SS ' },
            { code: 'JT', prefix: 'NP JS ' },
            { code: 'SH', prefix: 'NP SL ' },
            { code: 'MK', prefix: 'NP IM ' },
            { code: 'WF', prefix: 'NP WF ' }
        ];
        var MAX_UNITS = 20;
        ...
        // Add per-unit columns (up to MAX_UNITS per product)
        NP_PRODUCTS.forEach(function(p) {
            for (var u = 1; u <= MAX_UNITS; u++) {
                COLUMNS.push(p.prefix + 'Brand_' + u);
                COLUMNS.push(p.prefix + 'Asset No_' + u);
                COLUMNS.push(p.prefix + 'Serial_' + u);
                COLUMNS.push(p.prefix + 'Status_' + u);
                COLUMNS.push(p.prefix + 'Note_' + u);
                COLUMNS.push(p.prefix + 'Plug_' + u);
            }
        });
        // Status tracking timestamps
        ['Acknowledged By','Acknowledge Date','Corrected By','Corrected Date','Closed By','Closed Date','Rejected By','Rejected Date','Reject Reason'].forEach(function(c){ COLUMNS.push(c); });

        // Ensure header row matches
        var lastCol = sheet.getLastColumn();
        var existHeaders = lastCol > 0 ? sheet.getRange(1, 1, 1, lastCol).getValues()[0].map(function(h){ return String(h).trim(); }) : [];
        var colMap = {};
        existHeaders.forEach(function(h, idx) { colMap[h] = idx + 1; });
        COLUMNS.forEach(function(c) {
            if (!colMap[c]) {
                var newColIdx = sheet.getLastColumn() + 1;
                sheet.getRange(1, newColIdx).setValue(c);
                colMap[c] = newColIdx;
            }
        });

        // Generate unique ID
        var id = 'NP-' + new Date().getTime();
        var newRow = sheet.getLastRow() + 1;

        function setField(colKey, value) {
            if (!colMap[colKey] || value === undefined || value === null) return;
            sheet.getRange(newRow, colMap[colKey]).setValue(value);
        }
        ...
        setField('AMM MTN',       form.ammMtn || '');
        setField('Status',        'Pending');
        setField('Has Broken NP', form.hasBrokenNP || 'No');
        setField('Broken Units',  form.brokenUnits || '');
        setField('Broken Units Detail', form.brokenUnitDetail || '');
        setField('Comments',      form.comments || '');
        setField('Attachments_JSON', '{}');

        NP_PRODUCTS.forEach(function(p) {
            var qty = parseInt(form[p.code + '_qty']) || 0;
            setField(p.code + ' Qty', qty);
            for (var u = 1; u <= qty; u++) {
                setField(p.prefix + 'Brand_' + u,   form[p.code + '_' + u + '_brand']   || '');
                setField(p.prefix + 'Asset No_' + u, form[p.code + '_' + u + '_asset']   || '');
                setField(p.prefix + 'Serial_' + u,  form[p.code + '_' + u + '_serial']  || '');
                setField(p.prefix + 'Status_' + u,  form[p.code + '_' + u + '_status']  || 'ใช้งาน');
                setField(p.prefix + 'Note_' + u,    form[p.code + '_' + u + '_note']    || '');
                setField(p.prefix + 'Plug_' + u,    form[p.code + '_' + u + '_plug']    || '');
            }
        });

        // Enrich area info from store lookup
        try {
            var storeMap = _buildStoreEnrichMap();
            var code = String(form.branchCode || '').trim();
            if (code && storeMap[code]) {
                var s = storeMap[code];
                if (!String(form.dmArea||'').trim()) setField('DM Area',  s.dmName);
                if (!String(form.cmArea||'').trim()) setField('CM Area',  s.cmName);
                if (!String(form.ammMtn||'').trim()) setField('AMM MTN',  s.ammMtn);
            }
        } catch(enrichErr) { Logger.log('NP submit enrich error: ' + enrichErr); }
