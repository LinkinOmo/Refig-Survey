...
/**
 * Check if a CMS/Work Doc Number already exists in BigC_Work_Orders.
 * @param {string} cmsNumber - The CMS number to check.
 * @param {string|null} excludeId - Work Order ID to exclude (use when editing).
 * @returns {{ duplicate: boolean, existingId?: string, existingProject?: string }}
 */
function checkBigCCmsDuplicate(cmsNumber, excludeId) {
    try {
        if (!cmsNumber || !String(cmsNumber).trim()) return { duplicate: false };
        var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("BigC_Work_Orders");
        if (!sheet) return { duplicate: false };
        var cmsTrim = String(cmsNumber).trim().toLowerCase();
        var data = sheet.getDataRange().getValues();
        for (var i = 1; i < data.length; i++) {
            var existing = String(data[i][14] || '').trim().toLowerCase(); // Col 15 = Work Doc Number
            if (!existing || existing !== cmsTrim) continue;
            if (excludeId && String(data[i][1]).trim() === String(excludeId).trim()) continue;
            return { duplicate: true, existingId: String(data[i][1]), existingProject: String(data[i][3] || '') };
        }
        return { duplicate: false };
    } catch (e) {
        return { duplicate: false, error: e.toString() };
    }
}
